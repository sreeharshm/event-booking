...
import { getAllBoking } from '../api/Allapi'; // Consider fixing typo to getAllBooking in backend/API
...
function Allbooking() {
    ...
    return (
        <div className="min-h-screen bg-[#1f2533] flex text-gray-100 font-sans">
            ...
            <header className="fixed top-0 left-0 z-50 w-full h-16 flex items-center justify-between px-4 md:px-8 bg-[#2b3144] border-b border-gray-700/50">
                <div className="flex items-center gap-3">
                    <button
                        onClick={() => setIsSidebarOpen(!isSidebarOpen)}
                        className="md:hidden text-gray-400 hover:text-white p-1 rounded-lg transition-colors focus:outline-none"
                        aria-label="Toggle navigation menu"
                    >
                        {isSidebarOpen ? <X size={22} /> : <Menu size={22} />}
                    </button>
                    <p className="text-lg md:text-xl font-black tracking-tight text-white uppercase">
                        EVENT<span className="text-[#df183a]">HUB</span>
                        <span className="hidden sm:inline-block text-xs font-medium text-gray-400 tracking-normal normal-case border-l border-gray-600 pl-2 ml-2">
                            Admin
                        </span>
                    </p>
                </div>
                ...
                <button
                    ...
                    className="flex items-center gap-2 border border-gray-600 text-gray-300 text-xs font-bold rounded-lg px-3.5 py-2 hover:bg-[#f84464] hover:text-white hover:border-[#f84464] transition-all"
                >
                    <LogOut size={14} />
                    <span>Logout</span>
                </button>
            ...
            <aside
                className={`fixed top-16 bottom-0 left-0 bg-[#2b3144] border-r border-gray-700/50 p-6 z-40 transition-transform duration-300 w-64
            ...
            >
                <nav className="space-y-2">
                    ...
                    <button
                        className="w-full flex items-center gap-3 px-4 py-3 bg-[#df183a] text-white rounded-xl font-bold shadow-md shadow-[#df183a]/20 transition-all text-left"
                    >
            ...
            {/* Main Content Area */}
            <main className="flex-1 md:ml-64 pt-20 md:pt-24 px-4 md:px-8 pb-12 w-full max-w-full overflow-x-hidden">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 md:mb-8">
                        ...
                        {!loading && bookings.length > 0 && (
                            <div className="text-xs font-bold text-gray-400 uppercase tracking-widest bg-[#2b3144] px-4 py-2.5 rounded-xl border border-gray-700/50 w-full sm:w-auto text-center">
                    ...
                    {loading ? (
                        ...
                        <div className="bg-[#2b3144] rounded-2xl border border-gray-700/50 shadow-xl p-12 md:p-16 text-center">
                                    ...
                                    <div key={booking.id} className="bg-[#2b3144] border border-gray-700/50 rounded-xl p-4 space-y-3">
                                        <div className="flex justify-between items-start gap-2">
                                            <div className="min-w-0 flex-1">
                                                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">User</p>
                                            ...
                                            <span className="font-mono text-[11px] font-black text-gray-400 bg-[#232838] px-2 py-1 rounded shrink-0">
                                                #{booking.id ? booking.id.toString().slice(-6).toUpperCase() : 'N/A'}
                                            </span>
                                        </div>

                                        <div className="pt-2 border-t border-gray-700/40">
                                            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Event Item</p>
